...
- Nach Exportpfad-Änderungen zusätzlich sagen, welcher Pfad für Claude Desktop erwartet wird, z. B. `~/Documents/Claude/<slug>/style-skill/`.
- Bei Bild-Einbettungsfragen nicht auf lokale `/Users/<name>/...`-Host-Pfade als Lösung verweisen. Prüfen, dass `get_board_style` einen `EMBEDDABLE_IMAGE_SOURCES`-Block liefert, der auf `embeddable-images.json` im Exportpaket zeigt. Die großen `data:image/jpeg;base64,...`-Werte gehören in diese Manifest-Datei, nicht direkt ins Tool-Result, sonst greift Claudes 1MB-Limit.
